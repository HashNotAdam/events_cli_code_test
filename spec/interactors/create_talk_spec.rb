...
RSpec.describe CreateTalk do
      ...
      expect(Log::Error).to have_received(:call).with(
        message: <<~MESSAGE.chomp
          Error: Talk is not valid.
                 Event "Unknown event" does not exist.
        MESSAGE
      )
    end
  ...
  context "when the start time is in the wrong format" do
    ...
    it "logs an error" do
      ...
      expect(Log::Error).to have_received(:call).with(
        message: <<~MESSAGE.chomp
          Error: Talk is not valid.
                 Could not understand start time "1900".
        MESSAGE
      )
    end
  ...
  context "when the end time is in the wrong format" do
    ...
    it "logs an error" do
      ...
      expect(Log::Error).to have_received(:call).with(
        message: <<~MESSAGE.chomp
          Error: Talk is not valid.
                 Could not understand end time "1000".
        MESSAGE
      )
    end
  ...
  context "when the start time does not preceed the end time" do
    ...
    it "logs an error" do
      ...
      expect(Log::Error).to have_received(:call).with(
        message: <<~MESSAGE.chomp
          Error: Talk is not valid.
                 The start time must preceed the end time.
        MESSAGE
      )
    end
    ...
    it "logs an error" do
      ...
      expect(Log::Error).to have_received(:call).with(
        message: "Error: Talk is not valid.\n" \
          "       Talk overlaps with First talk (9:00am – 10:00am)."
      )
    end
  ...
  context "when the talk times overlap a different event" do
    before do
      Events << Event.new(name: "Different event")
      described_class.(
    ...
    it "adds a new talk to the Talks repository" do
      expect { call }.to change { Talks.all.count }.by(1)
    end
  end
  ...
  context "when the speaker does not exist" do
    ...
    it "logs an error" do
      ...
      expect(Log::Error).to have_received(:call).with(
        message: <<~MESSAGE.chomp
          Error: Talk is not valid.
                 Speaker "Unknown speaker" does not exist.
        MESSAGE
      )
    end
